...
int main() {
...
#endif
  {
    ...
    cv::Point2d pp;
    cv::calibrationMatrixValues(intrinsic_matrix_d,
                                cv::Size(img_undistort.cols, img_undistort.rows),
                                0.0,
                                0.0,
                                fovx,
                                fovy,
    ...
    cv::Mat E, R, t, mask;
    cv::Mat R1, R2, trans;
    std::vector<cv::Point2f> prev_points(0), current_points(0), current_points_device(0), prev_points_normalized(0),
        current_points_normalized(0), prev_points_device(0);
      ...
      if (current_points.size() >= num_ref_frames) {
        ...
        if (matching_rate > 0.1) {
          ...
          for (int ii = 0; ii < pnts3D.cols; ++ii) {
            ...
            double pix_color =
                img.at<uint8_t>(current_points_device[point_indices[ii]].y, current_points_device[point_indices[ii]].x);
          ...
          for (int ii = 0; ii < pnts3D.cols; ++ii) {
...

            {
              ...
              cv::circle(img_color,
                         cv::Point(current_points_device[point_indices[ii]].x,
                                   current_points_device[point_indices[ii]].y),
                         3,
                         color_pt);
            } else {
