...
Eigen::Vector2d reproject_point(const Eigen::Matrix3d &cam_rotation_mat,
  ...
  point_in_device_frame =
      Eigen::Vector2d(point_in_device_tmp[0] / point_in_device_tmp[2], point_in_device_tmp[1] / point_in_device_tmp[2]);
...
#define SHOW_RESULTS
...
#endif
  {
    ...
    for (size_t index_current_feature = 0; index_current_feature < feature_points_current.featureIDs.size();
         index_current_feature++) {
      ...
      frame_current.observingFeaturePointInDevice[current_feature_id] =
          {current_feature_position_in_device[0], current_feature_position_in_device[1]};
...
//                double match_rate = initializer::utils::initialize_feature_points(reference_frame, current_frame, localized_landmarks);
        double match_rate =
            initializer::utils::initialize_feature_points(reference_frame, current_frame, localized_landmarks,
...
//                    cv::waitKey(0);
          ...
          // 初期化後、前フレームカメラ位置の描画
          for (size_t initializing_frame_index = 2; initializing_frame_index < system_init_frame_id;
            ...
            cv::Affine3d tmp_cam_pose(tmp_camera_attitude,
                                      cv::Vec3f(tmp_frame.cameraPosition[0],
                                                tmp_frame.cameraPosition[1],
    ...
    if (is_initialized) {
      ...
      initializer::utils::estimate_frame_pose_pnp(frame_current,
                                                  database_landmark,
      ...
      cv::Affine3d current_cam_pose(current_camera_attitude,
                                    cv::Vec3f(database_frame[i].cameraPosition[0],
