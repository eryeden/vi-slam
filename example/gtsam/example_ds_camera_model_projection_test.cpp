...
int main(int argc, char* argv[]) {
  ...
  //! 観測値の生成
  vector<vector<Point2>> measurements;
  ...
  for (const auto& pose : poses) {
    ...
    for (const auto& point : points) {
      ...
      pc_source.emplace_back(point);
    }
    measurements.emplace_back(current_measurements);

    vslam::viewer::PointCloudPrimitive pcp("pc", pc_source);
    viewer.PushPrimitive(pcp);

    viewer.PushPrimitive(vslam::viewer::CoordinateSystemPrimitive(
        "world_origin" + std::to_string(pose.x()),
        vslam::Vec3_t(pose.x(), pose.y(), pose.z()),
        vslam::Quat_t(pose.rotation().matrix())

            ));

    cv::Mat img(double_sphere_camera_model.width_,
                double_sphere_camera_model.height_,
                CV_8UC3);
    img = 0;
    int32_t id_count = 0;
    for (const auto& m : current_measurements) {
      cv::circle(img, cv::Point(m.x(), m.y()), 2, cv::Scalar(255, 0, 0), 1);
      cv::putText(img,
  ...
  //  // problem is still under-constrained Here we add a prior on the position
  //  of
  //  // the first landmark. This fixes the scale by indicating the distance
  //  between
  //  // the first camera and the first landmark. All other landmark positions
  //  are
