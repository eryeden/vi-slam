...
int main(int argc, char* argv[]) {
  ...
  /// SmartFactorに被観測情報を登録する。
  /// ソースコードから見ると、一つのランドマークに対して、ランドマークを観測しているカメラIDを
  /// 登録する、とおう流れになっている。
  /// カメラIDは、`smartfactor->add(measurement,
  /// i);`とのときの番号`i`がIDになる。 Landmark
  /// IDは、SmartFactor内部で管理されるのだろうが、今のところ不明
  ...
  /// X1の位置を初期位置に拘束することで、スケール不定性を排除する。
  /// L0の位置を初期位置に拘束することでもOK。
  /// SmartFactorではLandmarkの位置をVariableとして扱っておらず、Factorの内部に持っているらしい。
  /// なので、陽にLandmark位置を拘束することができない。と思うMC_Kazである。
  ...
  graph.emplace_shared<PriorFactor<Pose3> >(
      1, poses[1], noise);  // add directly to graph
  ...
  /// Landmark位置はVariableとして扱われておらず、Factor内部にパラメータとして存在するので、
  /// 陽にFactorにアクセスする必要がある。
